...
mod test {
    ...
    fn test_initiate_swap_rejects_invalid_verifier() {
        let env = Env::default();
        env.mock_all_auths();
        ...
        // Pass a random address that is NOT the trusted zk_verifier from Config
        let untrusted_verifier = Address::generate(&env);
        client.initiate_swap(
            &listing_id,
            &buyer,
            &seller,
